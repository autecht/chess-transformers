...
find . -name "Carlsen.pgn" | sort > filenames.txt

# Combine all these files into a single file
./pgn_extract/pgn-extract -ffilenames.txt --output combined.pgn
...
for i in {1..6}
do
    ./pgn_extract/pgn-extract -Wfen $i.pgn --notags --noresults --output $i.fens
done
...
for i in {1..6}
do
    ./pgn_extract/pgn-extract -Wlalg $i.pgn --notags --nomovenumbers --nochecks -w7 --output $i.moves
done
